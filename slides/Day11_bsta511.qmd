...
## Software: __sample size__ for one mean test

* `power.t.test()` is from base R
    * `delta` is $\mu-\mu_0$ 
...
## Software: __power__ for one mean test
...
## Two-sample t-test: __sample size__

See section 5.4.4 of text for two-sample t-test sample size and power formulas using the normal distribution. 

* Suppose the data collected for the caffeine taps study were pilot day for a larger study. Investigators want to know what sample size they would need to detect a 2 point difference between the two groups. 
* Use base R's `power.t.test`, with 
    * `delta` the difference in means between the two groups that one would want to be able to detect as being significant,
    * `sd` is assumed to be the same in both groups
